...
describe("ProvablyFairService", () => {
  describe("hashSeed", () => {
    it("returns 64-char hex string", () => {
      const hash = ProvablyFairService.hashSeed(SERVER_SEED);
      expect(hash).toHaveLength(64);
      expect(hash).toMatch(/^[0-9a-f]+$/);
    });

    it("is deterministic", () => {
      expect(ProvablyFairService.hashSeed(SERVER_SEED)).toBe(
        ProvablyFairService.hashSeed(SERVER_SEED),
      );
    });

    it("differs for different seeds", () => {
      expect(ProvablyFairService.hashSeed("seed-a")).not.toBe(
        ProvablyFairService.hashSeed("seed-b"),
      );
    });
  });
  ...
  describe("calculateCrashPoint", () => {
    it("returns CrashPoint >= 1.00x (100)", () => {
      const cp = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
      expect(cp.valueX100).toBeGreaterThanOrEqual(100);
    });

    it("is deterministic — same seeds always produce same crash point", () => {
      const cp1 = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
      const cp2 = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
    ...
    it("different seeds produce different crash points (statistically)", () => {
      ...
      for (let i = 0; i < 20; i++) {
        const cp = ProvablyFairService.calculateCrashPoint(SERVER_SEED, `client-seed-${i}`);
    ...
    it("house edge: some rounds crash at exactly 1.00x", () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const cp = ProvablyFairService.calculateCrashPoint(`seed-${i}`, CLIENT_SEED);
  ...
  describe("verify", () => {
    it("returns true for correct crash point", () => {
      const cp = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
      expect(ProvablyFairService.verify(SERVER_SEED, CLIENT_SEED, cp.valueX100)).toBe(true);
    });

    it("returns false for wrong crash point", () => {
      const cp = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
      expect(ProvablyFairService.verify(SERVER_SEED, CLIENT_SEED, cp.valueX100 + 1)).toBe(false);
    });

    it("returns false when server seed is wrong", () => {
      const cp = ProvablyFairService.calculateCrashPoint(SERVER_SEED, CLIENT_SEED);
      expect(ProvablyFairService.verify("wrong-seed", CLIENT_SEED, cp.valueX100)).toBe(false);
    });
  });
  ...
  describe("crash point distribution sanity", () => {
    it("most crash points are above 1.00x", () => {
      ...
      for (let i = 0; i < total; i++) {
        const cp = ProvablyFairService.calculateCrashPoint(`server-${i}`, `client-${i}`);
